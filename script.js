 ...
 window.onload = ()=>{ //once window loaded
    for (let i = 0; i < allBox.length; i++) { //add onclick attribute to all avaliable section's spans
...
let playerXIcon = "bx bx-x"; //class name of fontawesome cross icon
let playerOIcon = "bx bx-circle"; //class name of fontawesome circle icon  
...
function clickedBox(element){
    // console.log(element);
    if(players.classList.contains("player")){ //if players elements has contain .player
        ...
        element.innerHTML = `<i class="${playerXIcon}"></i>`; //adding cross icon tag inside user clicked elemnet
        players.classList.add("active");
        element.setAttribute("id", playerSign)
    }
...
function bot(runBot){
    if(runBot){ //if runbot is true then run the following codes
        //first change the playerSign ... so if user has X vaule in id then bot will have O
    playerSign = "O"; 
    let array = []; //creating empty array...we'll store unselected box index in ths array
    ...
    if(array.length > 0){
        if(players.classList.contains("player")){ //if players elements has contain .player
            ...
            //if user is O then thebox id vaule will be X
            playerSign = "X";
            allBox[randomBox].setAttribute("id", playerSign);
        }else{
            allBox[randomBox].innerHTML = `<i class="${playerOIcon}"></i>`; //adding crircle icon tag inside user clicked elemnet
            players.classList.remove("active");
            allBox[randomBox].setAttribute("id", playerSign);
        }
...
function selectWinner(){ //if one condition of them matched then select the winner
...
replayBtn.onclick = ()=>{
    window.location.reload(); //reload the currents page 
}